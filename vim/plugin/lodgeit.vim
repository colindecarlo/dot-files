...
srv = ServerProxy('http://paste.2ndsiteinc.com/xmlrpc/', allow_none=True)
...
def paste_id_from_url(url):
    regex = re.compile(r'^http://paste.2ndsiteinc.com/show/([^/]+)/?$')
    m = regex.match(url)
    if m is not None:
        return m.group(1)
...
# new paste or reply
    ...
    url = 'http://paste.2ndsiteinc.com/show/%s' % paste_id
